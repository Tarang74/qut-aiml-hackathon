  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.card}>
        ...
        <div style={s.statGrid}>
          <StatCell label="CornCo Price" value={`$${parseFloat(state.price).toFixed(2)}`} color="#1d6b1d" />
...
function describeEvent(ev: GameEvent): string {
  switch (ev.kind) {
    ...
    case "drought":           return `Drought declared — ${ev.cycles} cycles of poor weather`;
    case "famine":            return "Famine — supply collapse across all farms";
    case "bumper_harvest":    return "Bumper harvest — exceptional yields this cycle";
    case "market_panic":      return "Market panic — prices in freefall";
    case "nuclear_fallout":   return "NUCLEAR FALLOUT — catastrophic destruction";
    ...
    case "corn_sold":         return `Corn sold — ${ev.bushels} bushels for $${parseFloat(ev.revenue).toFixed(0)}`;
    case "option_expired":    return `Option expired — PnL $${parseFloat(ev.pnl).toFixed(0)}`;
    case "player_joined":     return `${ev.name} joined the game`;
...
const s = {
  root: {
    ...
    padding: "1rem", width: "100%", minHeight: "100vh", background: "#f5f3ef", fontFamily: "inherit",
  ...
  cycleTag: {
    fontSize: "1.5rem", fontWeight: "700" as const, color: "#7a5010", letterSpacing: "0.04em",
  },
  waiting: {
    fontSize: "0.8rem", color: "#9a9a90", margin: "0.4rem 0 0", letterSpacing: "0.06em",
    textTransform: "uppercase" as const,
  },
  card: {
    background: "#ffffff", border: "1px solid #ddd9d2", borderRadius: 8, padding: "0.9rem 1rem",
  },
  feedbackCard: {
    background: "#edf8ed", border: "1px solid #9ac89a", borderRadius: 8, padding: "0.9rem 1rem",
  },
  headlineCard: {
    background: "#ffffff", border: "1px solid #ddd9d2", borderRadius: 8, padding: "0.75rem 1rem",
  },
  cardTitle: {
    margin: "0 0 0.6rem", fontSize: "0.7rem", color: "#8a8a80",
  ...
  eventList: { display: "flex", flexDirection: "column" as const, gap: "0.3rem" },
  eventRow: {
    margin: 0, fontSize: "0.85rem", color: "#4a4a44", padding: "0.3rem 0",
